---
hide:
  - navigation
  - toc
---

<div class="hero" markdown>

# Globus at the Smithsonian

<p class="hero-subtitle">Secure, high-performance data transfers for research</p>

<p class="hero-description">Globus is a data transfer platform that enables Smithsonian staff to move large datasets between storage systems and share data with external collaborators. Whether you're transferring terabytes of genomic data, archival video, high-resolution imagery, or field research files, Globus handles it reliably — even if you close your laptop mid-transfer. Explore the resources below to get started with Globus at the Smithsonian.</p>

<div class="hero-buttons" markdown>

[Log in to Globus :material-open-in-new:](https://app.globus.org){ .md-button .md-button--primary }
[Download Complete Guide (PDF)](pdfs/globus-complete-guide.pdf){ .md-button }

</div>

<div class="grid cards" markdown>

-   :material-book-open-variant:{ .lg .middle } **Basics**

    ---

    New to Globus? Start here.

    [:octicons-arrow-right-24: Get started](guide1-globus-basics.md)

-   :material-share-variant:{ .lg .middle } **Sharing**

    ---

    Share with external collaborators.

    [:octicons-arrow-right-24: Learn more](guide2-sharing-data.md)

-   :material-laptop:{ .lg .middle } **Personal**

    ---

    Transfer data from your laptop.

    [:octicons-arrow-right-24: Set up](guide3-globus-connect-personal.md)

-   :material-cog-sync:{ .lg .middle } **Advanced**

    ---

    Automate with Sync & Flows.

    [:octicons-arrow-right-24: Explore](guide4-advanced-features.md)

</div>

</div>

## Quick Reference

<div class="quick-ref-grid" markdown>

<div class="quick-ref-box" markdown>

### Get Storage Access

Request access to Smithsonian storage systems:

- **Hydra (HPC):** [Submit a request via SI Service Portal](https://smithsonianprod.servicenowservices.com/si?id=sc_cat_item&sys_id=962e05331b96e05078932f41f54bcb3b)
- **DAMS NAS:** Email SI-Globus@si.edu
- **STRI:** Email STRIhelp@si.edu

</div>

<div class="quick-ref-box" markdown>

### Quick Links

- :material-web: [Globus Web App](https://app.globus.org)
- :material-download: [Download Globus Connect Personal](https://www.globus.org/globus-connect-personal)
- :material-book-open-page-variant: [Official Globus Docs](https://docs.globus.org)

</div>

</div>

## Need Help?

<p style="text-align: center;" markdown>
**General Support:** SI-Globus@si.edu · **Hydra:** SI-HPC@si.edu · **STRI:** STRIhelp@si.edu
</p>

---

<p style="text-align: center; color: #888; font-size: 0.85rem; margin-bottom: 3rem;">
Maintained by the Office of Digital and Innovation · Smithsonian Institution
</p>